...
use Automattic\WooCommerce\Api\Enums\CouponStatus;
...
class ListCoupons {
	...
	public function execute(
		...
		$query_args = array(
			...
			'posts_per_page' => $limit + 1, // fetch one extra to determine has_next/prev
			'orderby'        => 'ID',
			'order'          => $last !== null ? 'DESC' : 'ASC',
		...
		if ( $after !== null ) {
			$after_id = (int) base64_decode( $after, true );
			$posts_query_args['post__not_in'] = array(); // Reset.
		...
		foreach ( $posts as $post ) {
			...
			$coupon    = CouponMapper::fromWcCoupon( $wc_coupon );
		...
		$page_info->has_next_page    = $last !== null ? ( $after !== null ) : $has_extra;
		$page_info->has_previous_page = $last !== null ? $has_extra : ( $after !== null );
