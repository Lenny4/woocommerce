...
use Automattic\WooCommerce\Api\Mutations\CreateCoupon as CreateCouponCommand;
...
class CreateCoupon {
	...
	public static function resolve(mixed $root, array $args, mixed $context, ResolveInfo $info): mixed {
		...
		if (array_key_exists('input', $args)) {
			$execute_args['input'] = self::convertCreateCouponInput($args['input']);
		}
  ...
  0 => 'manage_woocommerce',
	...
	private static function convertCreateCouponInput(array $data): \Automattic\WooCommerce\Api\InputTypes\CreateCouponInput {
		$input = new \Automattic\WooCommerce\Api\InputTypes\CreateCouponInput();
		...
		if (array_key_exists('discount_type', $data)) {
			$input->discount_type = $data['discount_type'] !== null ? \Automattic\WooCommerce\Api\Enums\DiscountType::from($data['discount_type']) : null;
		}
		if (array_key_exists('amount', $data)) {
			$input->amount = $data['amount'];
		}
		if (array_key_exists('status', $data)) {
			$input->status = $data['status'] !== null ? \Automattic\WooCommerce\Api\Enums\CouponStatus::from($data['status']) : null;
		}
